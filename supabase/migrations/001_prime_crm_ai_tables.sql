...
-- candidate_profiles
CREATE POLICY "candidate_profiles: admin full" ON public.candidate_profiles
  FOR ALL USING (public.current_user_role() = 'admin');
CREATE POLICY "candidate_profiles: counselor own students" ON public.candidate_profiles
  FOR ALL USING (public.current_user_role() = 'counselor' AND student_id IN (SELECT public.counselor_student_ids()));
CREATE POLICY "candidate_profiles: student reads own" ON public.candidate_profiles
  FOR SELECT USING (student_id = public.current_student_id());

-- evaluation_scores
CREATE POLICY "evaluation_scores: admin full" ON public.evaluation_scores
  FOR ALL USING (public.current_user_role() = 'admin');
CREATE POLICY "evaluation_scores: counselor own students" ON public.evaluation_scores
  FOR ALL USING (public.current_user_role() = 'counselor' AND student_id IN (SELECT public.counselor_student_ids()));
CREATE POLICY "evaluation_scores: student reads own" ON public.evaluation_scores
  FOR SELECT USING (student_id = public.current_student_id());

-- generated_cvs
CREATE POLICY "generated_cvs: admin full" ON public.generated_cvs
  FOR ALL USING (public.current_user_role() = 'admin');
CREATE POLICY "generated_cvs: counselor own students" ON public.generated_cvs
  FOR ALL USING (public.current_user_role() = 'counselor' AND student_id IN (SELECT public.counselor_student_ids()));
CREATE POLICY "generated_cvs: student reads own" ON public.generated_cvs
  FOR SELECT USING (student_id = public.current_student_id());

-- interview_prep
CREATE POLICY "interview_prep: admin full" ON public.interview_prep
  FOR ALL USING (public.current_user_role() = 'admin');
CREATE POLICY "interview_prep: counselor own students" ON public.interview_prep
  FOR ALL USING (public.current_user_role() = 'counselor' AND student_id IN (SELECT public.counselor_student_ids()));
CREATE POLICY "interview_prep: student reads own" ON public.interview_prep
  FOR SELECT USING (student_id = public.current_student_id());

-- job_leads: counselors and admin only
...
-- scan_history: admin only
CREATE POLICY "scan_history: admin full" ON public.scan_history
  FOR ALL USING (public.current_user_role() = 'admin');

-- scanner_config: admin only
CREATE POLICY "scanner_config: admin full" ON public.scanner_config
  FOR ALL USING (public.current_user_role() = 'admin');
